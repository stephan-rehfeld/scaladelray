...
import scaladelray.math.d.{Direction3, Point2, Point3}
...
class OldOrthographicCameraSpec extends FunSpec {
  ...
  describe( "An OldOrthographicCamera") {
    it( "should calculate the correct center for an image with the size of 1024x768 and a size of 1") {
      val cam = OrthographicOldCamera( Point3( 0, 0, 0 ), Direction3( 0, 0, -1 ), Direction3( 0, 1, 0 ), 1024, 768, 1 )
    ...
    it( "should calculate the correct number of rays for the given sampling pattern" ) {
      val cam1 = OrthographicOldCamera( Point3( 0, 0, 0 ), Direction3( 0, 0, -1 ), Direction3( 0, 1, 0 ), 400, 400, 400 )
      assert( cam1( 0, 0 ).size == 1 )


      val cam2 = OrthographicOldCamera( Point3( 0, 0, 0 ), Direction3( 0, 0, -1 ), Direction3( 0, 1, 0 ), 400, 400, 400, SamplingPattern.regularPattern( 3, 2 ) )
    ...
    it( "should use the points of the sampling pattern to generate correct altered ray of the primary ray of the pixel" ) {
      ...
      val cam = OrthographicOldCamera( Point3( 0, 0, 0 ), Direction3( 0, 0, -1 ), Direction3( 0, 1, 0 ), 400, 400, 400, sp )
